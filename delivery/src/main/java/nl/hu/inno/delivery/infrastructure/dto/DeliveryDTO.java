...
public record DeliveryDTO(UUID id, String timeLeft, String riderName, String orderLink) {
    public static DeliveryDTO toDTO(Delivery delivery) {
        return new DeliveryDTO(delivery.getDeliveryId(), DeliveryDTO.getTimeLeft(delivery.getEstimatedDelivery()), delivery.getRider().getName(), String.format("http://localhost:8080/orders/%s", delivery.getOrder().id()));
    }
